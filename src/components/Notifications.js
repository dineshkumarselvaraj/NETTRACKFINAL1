...
<Popup className="popupChild"
    trigger={<MenuItem className= "notifyDropDown" onClick={()=> this.handleClickOpen(data)}  > 
{/* onClick={()=> this.handleClickOpen(data)} */}
                        {!data.isOpened  ? 
                        
                         (<b>You have new notification ( Request Id : {data.requestID} ) <br/>
                         {/* Subject: {data.subject} */}
                         </b>)
                         :  (<div>You have new notification ( Request Id : {data.requestID} ) <br/>
                         {/* Subject: {data.subject} */}
                         </div>)
                        }
                        </MenuItem>}
    ...
    {close => (
      <div className="modal">
        ...
        <div className="newFontClass">
              <div>Ticket ID  : {data.requestID}</div> <br/>
              <label>Subject : </label> {data.subject} <br/>
              <label>Comments : </label> {data.comments} <br/>
              <label>Replied On : </label> {data.createdDate} <br/><br/>
             {/* { data.status !== 1  (<label>AssignedTo : </label> {data.assignedTo} <br/> )} */}
              {/* <label>Department : </label> {data.department} <br/> */}
              <ExpandCollapse>
              <ExpansionPanel>
                  <ExpansionPanelSummary expandIcon={<ExpandMoreIcon />}>
                    <Typography className={classes.heading}>Sender Info</Typography>
                  ...
                  <ExpansionPanelDetails>
                    <Typography>
                      <form >
                        ...
                        defaultValue={data.senderContactInfo}
                        className={classes.textField}
                        margin="normal"
                        InputProps={{
                          readOnly: true,
