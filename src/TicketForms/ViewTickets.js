...
class ViewTickets extends Component {
    ...
    loadTicketData = (requestID,partnerID) => {
      console.log('**********loadTicketData Method called*********');
      console.log('**********RequestID*********'+requestID);
      this.callGetServices(requestID,partnerID);
    }

    callGetServices = async (requestID,partnerID) => {
     ...
     await axios({
        ...
        this.callNetworkOperator(partnerID)
  ...
  render(){
    var {isLoaded,technicianAssignedData,currentData} = this.state;
    const { classes } = this.props;
  
    if(!isLoaded){
      return <div>Loading...</div>
    }else if(technicianAssignedData.length === 0){
      return <Typography variant="h5" color="inherit" className={classes.grow} align="center">No Record Found</Typography>
    }else{
      return (
      <div className={classes.root}>  
        <Paper className={classes.root}>
        <Typography variant="h6" color="inherit" className={classes.grow} align="left">Your Ticket Queue</Typography>
        ...
        <Table className={classes.table}>
          ...
          <TableBody>
            ...
            {technicianAssignedData.map(data => (
              <TableRow className={classes.row} key={data.requestID}>
                ...
                {/* <Button color="inherit" align="left" onClick={() => this.openStatusDialog(data)} >View</Button>  */}
                      <Button color="inherit" align="left" className = {"hideUnderLine"} 
                      onClick={()=> this.loadTicketData(data.requestID,data.complaintAgainst)}>View
                      {/* <Button color="inherit" align="left" className = {"hideUnderLine"} 
                      onClick={()=> this.setRequestId(data.requestID)}>View */}
      ...
      {/* <Switch>
          <Route  path = "/ManageTickets" render={(props) => <ManageTickets stateVal = {this.state} ticketID = {this.state.ticketID} isAuthed={true}  />}></Route>
      </Switch> */}
    </div>  
    );
    }
